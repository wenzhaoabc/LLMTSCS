...
from .trafficr1 import TrafficR1
from . import config
import wandb
import copy
import numpy as np
...
def pipeline_wrapper(
    dic_agent_conf, dic_traffic_env_conf, dic_path, roadnet, trafficflow
):
    results_table = []
    all_rewards = []
    all_queue_len = []
    ...
    for i in range(1):
        ...
        results_table.append(
            [
                round_results["test_reward_over"],
                round_results["test_avg_queue_len_over"],
        ...
        # delete junk
        cmd_delete_model = 'find <dir> -type f ! -name "round_<round>_inter_*.h5" -exec rm -rf {} \;'.replace(
            "<dir>", dic_path["PATH_TO_MODEL"]
        ).replace(
            "<round>", str(int(dic_traffic_env_conf["NUM_ROUNDS"] - 1))
        )
        cmd_delete_work = (
            'find <dir> -type f ! -name "state_action.json" -exec rm -rf {} \;'.replace(
                "<dir>", dic_path["PATH_TO_WORK_DIRECTORY"]
            )
        )
    ...
    results_table.append(
        [
            ...
            np.average(all_queue_len),
            np.average(all_travel_time),
        ]
    )
    results_table.append(
        [np.std(all_rewards), np.std(all_queue_len), np.std(all_travel_time)]
    )
...
def oneline_wrapper(
    ...
    for i in range(1):
        ...
        oneline = OneLine(
            dic_agent_conf=dic_agent_conf,
            dic_traffic_env_conf=merge(
                config.dic_traffic_env_conf, dic_traffic_env_conf
            ),
