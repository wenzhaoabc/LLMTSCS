...
def parse_args():
    ...
    parser.add_argument("--typ", type=str, default="s2c", choices=["c2s", "s2c"], help="CityFlow2SUMO or SUMO2CityFlow")
    parser.add_argument("--f", type=str, default="net", help="net or traffic")
    # sumo2cityflow
    parser.add_argument("--or_sumonet", type=str, default="grid4x4/grid4x4.net.xml")
    parser.add_argument("--cityflownet", type=str, default="grid4x4/grid4x4_roadnet_red.json")
    parser.add_argument("--or_sumotraffic", type=str, default="grid4x4/grid4x4.rou.xml")
    parser.add_argument("--cityflowtraffic", type=str, default="grid4x4/grid4x4_flow.json")
    parser.add_argument("--sumocfg", type=str, default="grid4x4/grid4x4.sumocfg")
    ...
    # parser.add_argument("--or_sumonet", type=str, default="cologne3/cologne3.net.xml")
    # parser.add_argument("--cityflownet", type=str, default="cologne3/cologne3_roadnet_red.json")
    # parser.add_argument("--or_sumotraffic", type=str, default="cologne3/cologne3.rou.xml")
    # parser.add_argument("--cityflowtraffic", type=str, default="cologne3/cologne3_flow.json")
    # parser.add_argument("--sumocfg", type=str, default="cologne3/cologne3.sumocfg")
...
def process_intersection_simple_phase(intersection):
    ...
    all_green = {"time": 30, "availableRoadLinks": intersection["trafficLight"]["roadLinkIndices"]}
    ...
    if node_type in ["traffic_light", "traffic_light_right_on_red"]:
        ...
        if SUMO_PROGRAM:
            ...
            for idx_phase in tls_dict[tlnodeid]._programs["0"]._phases:
                ...
                phase_dict = {"availableRoadLinks": list(set(index_list)), "time": duration}
    ...
    for edge in edges:
        ...
        if TRUE_CORRECTION_lane:
            for _v in edge._lanes:
                road["lanes"].append({"width": _v._width, "maxSpeed": _v._speed})
        else:
...
def sumo2cityflow_flow(args):
    ...
    sumofile = args.or_sumotraffic
    cityflowfile = args.cityflowtraffic
    ...
    if root.find("trip") != None and "rou" in sumofile:
        ...
        os.system(f"duarouter --route-files={dst} --net-file={sumonet} --output-file={sumofile}")
    ...
    if root.find("vType") != None:
        ...
        width = float(root.find("vType").attrib["width"]) if "width" in root.find("vType").attrib else 2.0
        maxPosAcc = float(root.find("vType").attrib["accel"]) if "accel" in root.find("vType").attrib else 2.0
    ...
    with open(cityflowfile, "w", encoding="utf-8") as f:
        f.write("[\n")
...
def sumo2cityflow_net(args):
    ...
    """
    # parent dir of current dir
    sumofile =  args.or_sumonet
    cityflowfile = args.cityflownet
    ...
    result = {"intersections": final_intersections, "roads": final_roads}
...
def cityflow2sumo_flow(args):
    ...
    for idx, info in enumerate(data):
        vehicle, route, interval, startTime, endTime = (
            info["vehicle"],
            info["route"],
            info["interval"],
            info["startTime"],
...
def cityflow2sumo_net(args):
    ...
    root_node.setAttribute("xsi:noNamespaceSchemaLocation", "http://sumo.dlr.de/xsd/nodes_file.xsd")
    ...
    root_con.setAttribute("xsi:noNamespaceSchemaLocation", "http://sumo.dlr.de/xsd/connections_file.xsd")
    ...
    root_tll.setAttribute("xsi:noNamespaceSchemaLocation", "http://sumo.dlr.de/xsd/tllogic_file.xsd")
    ...
    for inter in data["intersections"]:
        ...
        node.setAttribute("type", str("priority") if inter["virtual"] else "traffic_light_right_on_red")
        ...
        for idx, link in enumerate(inter["roadLinks"]):
            ...
            for lanelink in link["laneLinks"]:
                ...
                con.setAttribute("fromLane", str(abs(start_num_lanelinks - lanelink["startLaneIndex"])))
                con.setAttribute("toLane", str(abs(end_num_lanelinks - lanelink["endLaneIndex"])))
            ...
            for idx, light in enumerate(inter["trafficLight"]["lightphases"]):
                ...
                if idx != 0 and light["availableRoadLinks"] is not None:  # idx=0 means yellow phase
                    ...
                    for act_roadlink in light["availableRoadLinks"]:
                        state[phase_dic[act_roadlink][0] : sum(phase_dic[act_roadlink])] = (
                            single_phase * phase_dic[act_roadlink][1]
                        )
                ...
                if light["time"] <= 5:
                    ...
                    assert idx == 0
                    for act_roadlink in light["availableRoadLinks"]:
                        yellow_state[phase_dic[act_roadlink][0] : sum(phase_dic[act_roadlink])] = ["s"] * phase_dic[
                            act_roadlink
                        ][1]
    ...
    root_edge.setAttribute("xsi:noNamespaceSchemaLocation", "http://sumo.dlr.de/xsd/edges_file.xsd")
    ...
    doc_edge.writexml(fp_edge, indent="\t", addindent="\t", newl="\n", encoding="utf-8")
    fp_edge.close()
